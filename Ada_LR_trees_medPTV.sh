...
filename=$(echo "Ada_${i}_${j}_${1}_medPTV.sh")
...
echo "RunMVATraining -d /unix/atlas3/avalee/MVA/SMU_1lep_truthtag_32-07/ -a 1 -s VH -m $1 -x $1 -l 75 -u 150 -H '!H:!V:NTrees=$j:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=$i:SeparationType=GiniIndex:nCuts=100:MinNodeSize=5:PruneMethod=NoPruning' -c 'Ada_${i}_${j}'" >> batch_submit/$filename

chmod a+xwr batch_submit/$filename

qsub -N "Ada_${i}_${j}_${1}_medPTV" -q medium -j oe batch_submit/$filename
